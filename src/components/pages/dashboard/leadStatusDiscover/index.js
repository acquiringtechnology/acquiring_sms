...
export const LeadStatusDiscover = ({ leadListData = {} }) => {
  const donutOptions = {
    chart: {
      type: 'donut'
    },
  ...
  const handleGetCurentMonthLeadDescover = (leadStatus) => {
    const candidateList = leadListData;
    if (candidateList && candidateList.length > 0) {
      return candidateList.filter(
        ({ status }) => Number(status) === Number(leadStatus)
      ).length;
    }
    return 0;
  };
