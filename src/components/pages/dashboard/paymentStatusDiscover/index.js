...
export const PaymentStatusDiscover = ({ invoiceListData = [] }) => {
    ...
    const handleGetCurentMonthLeadDescover = (status) => {
        ...
        if (invoiceListData && invoiceListData.length > 0) {

          if(PAYMENT_STATUS.COMPLETED === status){
            const filteredInvoices = invoiceListData.filter(
              ({ paymentStatus }) => Number(paymentStatus) === Number(status)
            );
            
            const totalPay = filteredInvoices.reduce((total, { billingInfo }) => {
              const fees = billingInfo?.reduce((sum, { payFees }) => sum + (Number(payFees) || 0), 0);
              return total + (fees || 0);
            }, 0);
          ...
          }else if(PAYMENT_STATUS.PENDING === status){
            const filteredInvoices = invoiceListData.filter(
              ({ paymentStatus }) => Number(paymentStatus) === Number(status)
            );
            const totalPay = filteredInvoices.reduce((total, { billingInfo,totfees=0 }) => {
              const fees = getPendingPayment(billingInfo,totfees);
            ...
            <div className="col-6 col-xl-4">
              <div className="d-flex flex-column flex-center align-items-sm-start flex-md-row justify-content-md-between flex-xxl-column p-3 ps-sm-3 ps-md-4 p-md-3 h-100 border-1 border-bottom border-end border-translucent">
                <div className="d-flex align-items-center mb-1">
                  ...
                  <span className="mb-0 fs-9 text-body"> {handleGetCurentMonthLeadDescover(PAYMENT_STATUS.COMPLETED)?.count} Completed</span>
                </div>
                <h3 className="fw-semibold ms-xl-3 ms-xxl-0 pe-md-2 pe-xxl-0 mb-0 mb-sm-3 text-primary">
                 
                ₹ {handleGetCurentMonthLeadDescover(PAYMENT_STATUS.COMPLETED)?.amount}
                </h3>
              </div>
            </div>
            ...
            <div className="col-6 col-xl-4">
              <div className="d-flex flex-column flex-center align-items-sm-start flex-md-row justify-content-md-between flex-xxl-column p-3 ps-sm-3 ps-md-4 p-md-3 h-100 border-1 border-bottom border-end border-translucent">
                <div className="d-flex align-items-center mb-1">
                  ...
                  {handleGetCurentMonthLeadDescover(PAYMENT_STATUS.PENDING)?.count} Pending
                  </span>
                </div>
                <h3 className="fw-semibold ms-xl-3 ms-xxl-0 pe-md-2 pe-xxl-0 mb-0 mb-sm-3 text-info">
               
                ₹ {handleGetCurentMonthLeadDescover(PAYMENT_STATUS.PENDING)?.amount}
                </h3>
              </div>
            </div>
            ...
            <div className="col-6 col-xl-4">
              <div className="d-flex flex-column flex-center align-items-sm-start flex-md-row justify-content-md-between flex-xxl-column p-3 ps-sm-3 ps-md-4 p-md-3 h-100 border-1 border-bottom border-translucent">
                <div className="d-flex align-items-center mb-1">
                  ...
                  <span className="mb-0 fs-9 text-body">
                   {handleGetCurentMonthLeadDescover(PAYMENT_STATUS.PARTIAL_PENDING)?.count} Partial Pending
                  </span>
                </div>
                <h3 className="fw-semibold ms-xl-3 ms-xxl-0 pe-md-2 pe-xxl-0 mb-0 mb-sm-3  text-warning">
                ₹ {handleGetCurentMonthLeadDescover(PAYMENT_STATUS.PARTIAL_PENDING)?.amount}
                </h3>
              </div>
            </div>
            ...
            <div className="col-6 col-xl-4">
              <div className="d-flex flex-column flex-center align-items-sm-start flex-md-row justify-content-md-between flex-xxl-column p-3 ps-sm-3 ps-md-4 p-md-3 h-100 border-1 border-end border-translucent">
                <div className="d-flex align-items-center mb-1">
                  ...
                  <span className="mb-0 fs-9 text-body">
                  {handleGetCurentMonthLeadDescover(PAYMENT_STATUS.REFUNDED)?.count} Refunded
                  </span>
                </div>
                <h3 className="fw-semibold ms-xl-3 ms-xxl-0 pe-md-2 pe-xxl-0 mb-0 mb-sm-3 text-success">
                ₹ {handleGetCurentMonthLeadDescover(PAYMENT_STATUS.REFUNDED)?.amount}
                </h3>
              </div>
            </div>
            ...
            <div className="col-6 col-xl-4">
              <div className="d-flex flex-column flex-center align-items-sm-start flex-md-row justify-content-md-between flex-xxl-column p-3 ps-sm-3 ps-md-4 p-md-3 h-100 border-1  border-end border-translucent">
                <div className="d-flex align-items-center mb-1">
                  ...
                  <span className="mb-0 fs-9 text-body">{handleGetCurentMonthLeadDescover(PAYMENT_STATUS.REFUNDED_PENDING)?.count} Refunded Pending</span>
                </div>
